function MinHeap() {
  ...
  this.heapifyUp = function () {
    ...
    // Continue while the index is not the root and the current element is less than its parent
    while (
      index > 0 &&
  ...
  this.remove = function () {
    ...
    const min = this.heap[0]; // Store the minimum element (root)
  ...
  this.heapifyDown = function () {
    ...
    while (true) {
      ...
      this.swap(index, smallestIndex); // Swap the current element with the smallest element
      index = smallestIndex; // Move index to the smallest index
    }
  };

  this.peek = function () {
    return this.heap[0]; // Return the minimum element (root) without removing it
  };
}
